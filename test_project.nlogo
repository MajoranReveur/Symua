...
to setup-constants
  set number-warriors 4
  set number-adventurers 30
  set number-monsters 2
  set number-spawns 5
  set number-switchs 10
  set number-chests 30
  set delay-between-spawns 30
  set delay 0
end
...
to setup-patches
  ...
  ask n-of number-spawns patches with [pcolor = black] [set pcolor red]
  ask n-of number-switchs patches with [pcolor = black] [set pcolor magenta]
  ask n-of number-chests patches with [pcolor = black] [set pcolor yellow]
end

to setup-adventurers
  ask patches with [pcolor = blue]
  [
    sprout-adventurers number-adventurers
    [
      set health 10
      set group 0
      set leader false
  ...
  if number-warriors > 0
  [
    ifelse number-warriors < (count adventurers) [ask n-of number-warriors adventurers [set warrior true]] [ask adventurers [set warrior true]]
  ]
  update-adventurers-visibility
  update-adventurers-groups
end
...
to go
  set delay delay + 1
  if delay = delay-between-spawns [setup-monsters
  set delay 0]
  ask adventurers [set pain false]
  ask monsters [set pain false]
  update-adventurers-groups
  ask adventurers [adventurer-move]
  update-adventurers-visibility
  ask monsters [monster-move]
  update-global-variables
  update-display
  tick
end
...
to update-adventurers-visibility
  ask adventurers
  [
    let mygroup group
    ask patches with [distance myself < 5] [set seen true
      if pcolor = black [set pcolor grey]]
  ]
end

to update-adventurers-groups
  ask adventurers
  [
    if follows != nobody
    [ if (distance follows) > 5 [ set follows nobody ]]
    if follows = nobody
    [
      let mygroup group
      let nearest min-one-of adventurers with [distance myself <= 5 and group != 0 and group != mygroup] [distance myself]
      ifelse nearest = nobody
      [
        if leader = false
        [
          set leader true
          set group number-groups + 1
          set number-groups number-groups + 1
          let tail_found false
          set nearest self
          while [tail_found = false]
          [
            let follower one-of adventurers with [follows = nearest]
            ifelse follower = nobody [set tail_found true] [
              ask follower [set group [group] of nearest
                set nearest follower]]
          ]
        ]
      ]
      [
        ...
        [
          let follower one-of adventurers with [follows = nearest]
          ifelse follower = nobody [set tail_found true] [set nearest follower]
        ]
        set group [group] of nearest
        set follows nearest
        if leader = true
        [ask adventurers with [group = mygroup] [set group [group] of nearest]]
        set leader false
      ]
    ]
  ]
...
to update-display
  ask adventurers with [warrior = true] [ if shape != warrior-shape [ set shape warrior-shape ]]
  ask adventurers with [warrior = false] [ if shape != ranger-shape [ set shape ranger-shape ]]
  ask adventurers [ifelse pain = true [set color red] [set color blue]]
  ask monsters [ if shape != monster-shape [ set shape monster-shape ]
    ifelse pain = true [set color red] [ifelse sound-target = nobody [set color green] [set color orange]]]
  stop-inspecting-dead-agents
end

;; Turtles move about at random.
to adventurer-move ;; turtle procedure
  let nearest min-one-of monsters with [distance myself < 5] [distance myself]
  ifelse (nearest != nobody)
  [
    ifelse warrior = true
    [ifelse distance nearest < 1 [warrior-attacks] [set heading (towards nearest)
      forward 1]]
    [ifelse distance nearest < 3 [set heading (towards nearest + 180)
      forward 1] [ranger-attacks]]
  ]
  [
    ifelse leader = true
    [
      set nearest min-one-of patches with [pcolor = yellow and distance myself < 5] [distance myself]
      ifelse (nearest != nobody)
      [
      ...
      [
        set nearest min-one-of patches with [pcolor = magenta and distance myself < 5] [distance myself]
        ifelse (nearest != nobody)
        [
          set heading (towards nearest)
          forward 1
        ...
        [
          set nearest min-one-of patches with [seen = false] [distance myself]
          ifelse (nearest != nobody)
          [
            set heading (towards nearest)
            forward 1
          ...
          [
            set nearest min-one-of patches with [pcolor = magenta and seen = true] [distance myself]
            ifelse (nearest != nobody)
            [
              set heading (towards nearest)
              forward 1
            ]
            [set nearest min-one-of monsters [distance myself]
            if nearest != nobody
            [
              set heading (towards nearest)
              forward 1
  ...
  ask patch-here [
    if pcolor = grey or pcolor = black [set pcolor white]
    if pcolor = yellow [set pcolor white
      ask monsters with [distance myself < 30] [set sound-target myself]]
    if pcolor = magenta [set pcolor white
      ask monsters with [distance myself < 30] [set sound-target myself]
      set number-switchs number-switchs - 1
      if number-switchs = 0
      [
        ask patches with [pcolor = red] [set pcolor black
          if seen = true [set pcolor grey]]
      ]
    ]
  ]
end
...
to monster-attacks
  let target min-one-of adventurers with [distance myself < 1] [distance myself]
  if target != nobody
  [ask target [lose-health]]
end

to warrior-attacks
  let target min-one-of monsters with [distance myself < 1] [distance myself]
  if target != nobody [ask target [lose-health]]
end

to ranger-attacks
  let target min-one-of monsters with [distance myself > 3 and distance myself < 5] [distance myself]
  if target != nobody [ask target [lose-health]]
end
...
to lose-health
  set health health - 1
  set pain true
  ask patch-here [ask monsters with [distance myself < 10] [set sound-target myself]]
  if health <= 0 [die]
end

to monster-move ;; turtle procedure
  let nearest min-one-of adventurers with [distance myself < 5] [distance myself]
  ifelse (nearest != nobody)
  [
    set sound-target nobody
    ifelse distance nearest < 1 [monster-attacks] [set heading (towards nearest)
    forward 1]
  ]
  [
    if patch-here = sound-target [set sound-target nobody]
    ifelse sound-target != nobody [set heading (towards sound-target)] [set heading random 360]
    forward 1]
end
